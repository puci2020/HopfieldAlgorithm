...
namespace DMU.Terminal {
  internal class Badanie {
    internal static void Uruchom() {
      StreamWriter file = new StreamWriter("Raport1.txt");
      ...
      double[] matrixData = Array.ConvertAll(array, el => double.Parse(el));
      ...
      for (int i = 0; i < vectors_I.GetLength(0); i++) {
        ...
        Matrix W = new Matrix(matrixData, dimension, dimension);
